...
# Save output
output_file = os.path.join(output_path, 'output_results.csv')
output_data = np.concatenate((y_pred.reshape(len(y_pred), 1), y_test.reshape(len(y_test), 1)), 1)
output_df = pd.DataFrame(output_data, columns=['Predicted', 'Actual'])
